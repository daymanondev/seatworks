...
test("a change to what a seat reads comes with a new version", { skip: git("rev-parse", "HEAD") === undefined && "not a git checkout" }, () => {
  ...
  if (dirty) {
    assert.notEqual(versionNow(), versionAt("HEAD"), `what a seat reads changed since the last commit; raise version in package.json:\n${dirty}`);
    return;
  }
  const parent = git("rev-parse", "--verify", "HEAD~1")?.trim();
  const moved = parent ? git("diff", "--name-only", "HEAD~1", "HEAD", "--", ...SEAT_FACING)?.trim() : "";
  if (moved) assert.notEqual(versionAt("HEAD"), versionAt("HEAD~1"), `the last commit changed what a seat reads without raising the version:\n${moved}`);
});
