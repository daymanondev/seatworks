...
export function makeKit(): Kit {
  ...
  // The shipped ecosystem, Paseo's tools and the watch's questions are the world's, not this fixture's to make up.
  for (const name of ["ecosystem.json", "paseo.json", "checks.json"]) put(dir, `catalog/${name}`, readFileSync(new URL(`../catalog/${name}`, import.meta.url), "utf-8"));
