...
test("a decision only the Human can make waits in their queue, and what they said in chat goes on record only with their own words", async () => {
  ...
  assert.match(asked.text, /^Asked the Human as H1; it waits in their question queue\. Nothing it decides goes ahead until they answer\. Lane L1 is on hold for it\./);
  assert.equal(h.ledger().lanes.L1!.onHold?.reason, "it waits for the Human's answer to H1: Delete old invoices, or keep them archived?");
  assert.match((await h.call(sup, "supervisor", "status", {})).text, /## Questions for the Human\n\n- H1 \(irreversible, L1\), open 0 min: Delete old invoices, or keep them archived\? Recommended: Archive\. While silent: The lane archives them/);
  ...
  assert.match((await h.call(sup, "supervisor", "record_human_answer", { question: "H1", choice: "decline", quote: "archive them" })).text, /H1 is already answered\./);
  assert.ok(lane.lead);
});
...
test("a question is put as a choice with a recommendation among its options, and the Human gets no more of them in a day than they allow", async () => {
  ...
  assert.match((await h.call(sup, "supervisor", "ask_human", packet({ recommend: "Shred" }))).text, /recommend names none of the options: give one of Delete, Archive\./);
  assert.match((await h.call(sup, "supervisor", "ask_human", packet({ options: [{ label: "Delete", effect: "x" }, { label: "cancel", effect: "y" }], recommend: "Delete" }))).text, /none called decline or cancel/);
  assert.match((await h.call(sup, "supervisor", "ask_human", packet({ options: [{ label: "Delete", effect: "x" }], recommend: "Delete" }))).text, /options takes at least 2/);
  for (const n of [1, 2, 3]) assert.match((await h.call(sup, "supervisor", "ask_human", packet())).text, new RegExp(`^Asked the Human as H${n};`));
  assert.match((await h.call(sup, "supervisor", "ask_human", packet())).text, /The Human has had 3 questions in the last day \(H1, H2, H3\), and 3 is what they allow/);
});
