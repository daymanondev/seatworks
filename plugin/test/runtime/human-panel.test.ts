...
test("a question waits on Flow, and the Human's choice there goes on the record and to whoever asked", async () => {
  ...
  assert.deepEqual(flow.questions.map((question) => [question.id, question.class, question.lane, question.recommend, question.options.map((option) => option.label)]), [["H1", "irreversible", "L1", "Archive", ["Delete", "Archive"]]]);
  assert.equal(flow.lanes[0]!.onHold?.reason, "it waits for the Human's answer to H1: Delete old invoices, or keep them archived?");

  const answer = (question: string, choice: string, note = "") => h.rpc(contracts.questionAnswer, { project: h.project.slug, question, choice, note });
  assert.deepEqual(await answer("H1", "Keep"), { error: "Keep is none of H1's options: Delete, Archive, or decline or cancel." });
...
test("Orders reads back the Human's standing orders and the project's concept, and says when the orders cannot be read", async () => {
  ...
  assert.deepEqual([orders.fault, orders.askFirst, orders.laneHome, orders.ownRules, orders.riskRules.length], [null, ["src/auth"], "isolate", false, 1]);
  assert.match(orders.riskRules[0]!.reviewQuestion, /second time/);
  assert.deepEqual([orders.concept?.text, orders.concept?.more], ["# Invoices\n\nAn invoice is never edited.\n", false]);

  await h.call(sup, "supervisor", "set_project", { riskRules: [] });
  const own = await h.rpc(contracts.orders, { project: h.project.slug });
  assert.ok("askFirst" in own && own.ownRules && own.riskRules.length === 0, "an empty list of its own is the project's answer, not the kit's");
...
test("the Report tells the last day from the record: what needs the Human, what went ahead, what landed, and what could not be undone", async () => {
  ...
  assert.deepEqual(report.needs.map((item) => [item.title, item.detail]), [["H1 · Delete old invoices, or keep them archived?", "irreversible · L1"]]);
  assert.deepEqual(report.ahead.map((item) => [item.title, item.detail]), [["H2 · Dates as ISO?", "reversible · went ahead on Archive"]]);
  assert.deepEqual(report.beyond.map((item) => [item.title, item.detail]), [["I1 · git push --force origin main", "the Peer on L1-T1 (Clean build) · not marked"]]);
  assert.deepEqual(report.numbers.map((row) => [row.title, row.value]), [["Questions today", "2 of 3"], ["Landings", "0 landed"], ["Incidents", "1"]]);
  ...
  await landing.call(boss, "supervisor", "open_lane", { title: "Cart", outcome: "a cart", acceptance: ["a"], outOfScope: ["the rest"] });
  ...
  assert.deepEqual(landed.landed.map((item) => [item.title, item.detail]), [["L1 Cart", "on main"]]);
});
