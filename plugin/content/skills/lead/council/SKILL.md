...
Keep it with `note` in council as `<case-id>.md`, not in the repository.
...
- The council ends at the verdict. Reviewers don't implement; a task in `add_tasks` carries the required action as goal and acceptance, the boundaries as owned paths and out of scope, and the verdict's decisions in context.
