...
export async function landCheck(project: Project, ledger: Ledger, lane: Lane, gate: LandGate, checks: Checkpoints): Promise<{ signals: string[]; evidence: string[] }> {
  ...
  const signals = [
    ...
    ...Object.values(loadIncidents(project.state).items)
      .filter((incident) => incident.open && incident.lane === lane.id)
      .map((incident) => `Incident ${incident.id} on this lane is still open: ${incident.kind}.`),
  ];
