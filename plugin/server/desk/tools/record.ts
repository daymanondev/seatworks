...
/** A seat's steps from its history, one numbered line each, with no output or diffs; what the desk kept once the seat is gone. */
export const record = defineTool({
  ...
  async handle(desk, caller, args) {
    const found = whose(loadLedger(caller.project.state), caller, args.of);
    if (typeof found === "string") return no(found);
    const limit = args.limit ?? STEPS;
