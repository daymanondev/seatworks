...
export const setProject = defineTool({
  name: "set_project",
  input: z.strictObject({ base: z.string().optional(), gate: z.string().optional(), gateTimeoutMinutes: z.number().optional(), gateOn: z.enum(["lane", "task"]).optional(), serialOnly: z.array(z.string()).optional(), landAs: z.enum(["squash", "merge", "ff"]).optional(), laneHome: z.enum(LANE_HOMES).optional(), askFirst: z.array(z.string()).optional() }),
  async handle(_desk, caller, args) {
    ...
    const next: ProjectConfig = {
      ...
      askFirst: Array.isArray(args.askFirst) ? strs(args.askFirst).map((path) => path.trim()).filter(Boolean) : config.askFirst,
    };
    saveConfig(caller.project.state, next);
    const home = next.laneHome ? `lanes open as ${next.laneHome} unless a call says otherwise` : "where a lane opens is asked when the Human's copy makes it a question";
    const asked = next.askFirst.length > 0 ? `a landing that touches ${next.askFirst.join(", ")} waits for the Human` : "no landing waits for the Human";
    return ok(`Base ${next.base ?? "unset"}; gate ${next.gate || "none"}, run per ${next.gateOn}; gate timeout ${next.gateTimeoutMinutes} minutes; lanes land as ${next.landAs}; ${home}; ${asked}.`);
  },
});
