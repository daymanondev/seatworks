...
import { findLane, loadLedger, nextQuestionId } from "../ledger.ts";
  ...
  async handle(desk, caller, args) {
    ...
    const opened = ctx.transact(project, (ledger) => {
      ...
      const question: Question = {
        ...
        status: "open",
        openedAt: Date.now(),
        parked: (args.class === "irreversible" && lane !== undefined) || undefined,
      };
    ...
    return ok(`Asked the Human as ${opened.id}; it waits in their question queue. ${WHILE_SILENT[opened.class]}${held} An answer they give you here goes on record with record_human_answer.`);
  },
});
