...
export const raise: Tool = async (services, caller, args) => {
  ...
  const answered = task !== undefined && ledger.lanes[task.lane]?.status === "open";
  if (!answered && !(await roster.seated(sent.seat.id))) return no(`${sent.seat.id} has gone, so nothing raised about it would reach anyone.`);
