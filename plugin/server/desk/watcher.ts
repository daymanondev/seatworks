...
const takes = (question: Question) => [...(question.type === "noul" ? ["yes", "no"] : Object.keys(question.criteria)), UNSURE];
...
export class Watcher {
  ...
  private deliver(project: Project, role: string, letter: Letter): Promise<string> {
    const next = (this.lines.get(project.slug) ?? Promise.resolve()).catch(() => undefined).then(() => this.deliverOne(project, role, letter));
  ...
  answer(caller: string, id: string, said: Said[]): string | undefined {
    ...
    const problems = [
      ...Object.keys(entry.questions).filter((name) => words.filter((one) => one.question === name).length !== 1).map((name) => `answer ${name} once`),
      ...words.flatMap((one) => {
        const question = asked(one.question);
        if (!question) return [`${one.question} is no question of ${id}`];
        return [...(takes(question).includes(one.says) ? [] : [`${one.question} takes ${takes(question).join(", ")}`]), ...(one.why ? [] : [`give ${one.question} a why`])];
      }),
    ];
    if (problems.length > 0) return `Nothing was recorded: ${[...new Set(problems)].join("; ")}.`;
    this.waiting.delete(id);
    entry.answered({ answers: Object.fromEntries(words.map((one) => [one.question, answerOf(asked(one.question)!, one.says)])), model: entry.model, why: Object.fromEntries(words.map((one) => [one.question, one.why])) });
    return undefined;
  }
