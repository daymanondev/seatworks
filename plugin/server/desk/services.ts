...
import type { Caller, DeskContext, ToolReply } from "./context.ts";
...
export function defineTool<Input extends z.ZodObject>(tool: { name: string; input: Input; handle(desk: DeskServices, caller: Caller, input: z.infer<Input>): Promise<ToolReply> }): ToolDef {
  return tool;
}
...
function structure(schema: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const key of Object.keys(schema).sort()) {
    ...
    if (key === "properties") kept[key] = Object.fromEntries(Object.entries(value as Record<string, Record<string, unknown>>).sort(([a], [b]) => a.localeCompare(b)).map(([name, field]) => [name, structure(field)]));
...
export function servedBy(tools: ToolDef[], name: string, shown: ArgSchema): ToolDef | undefined {
  ...
  return tools.find((tool) => tool.name === name && shapeOf(tool, () => z.toJSONSchema(tool.input, { io: "input" })) === wanted);
}
