...
export type Moment = { facts: { kind: string; quote: string }[]; instruction?: { text: string; from: string[] }; turn: string | null };
...
export function momentCases(kit: Kit, place: { lane?: Lane; task?: Task }, moment: Moment): Case[] {
  ...
  const work = task ? { subject: task.id, goal: task.goal, acceptance: task.acceptance, out_of_scope: task.outOfScope } : lane && { subject: lane.id, goal: lane.outcome, acceptance: lane.acceptance, out_of_scope: lane.outOfScope };
  if (!work) return [];
  ...
  const handback = task?.handback && moment.facts.some((found) => found.kind === "unverified" || found.kind === "claim-contradicted") ? handbackOf(task) : undefined;
  if (task?.handback && handback) cases.push({ subject: task.id, episode: basename(task.handback.file), state: { handback }, asked: { claims_checks_pass: { check: "claims_checks_pass" } } });
  ...
  if (instruction && moment.facts.some((found) => found.kind === "edit-before-look") && (!after || from.some((sender) => after.includes(sender)))) {
    cases.push({ subject, episode, state: { instruction }, asked: { instruction_kind: { check: "instruction_kind" } } });
  }
  return cases;
}
