...
function tasksByLane(ledger: Ledger, seats: Map<string, SeatView>, now: number, open: ReadonlySet<string>): { counts: Map<string, Counted>; held: Map<string, FlowTask[]> } {
...
function laneOf(lane: Lane, ledger: Ledger, seats: Map<string, SeatView>, now: number, count: Counted, tasks: FlowTask[], open: boolean): FlowLane {
  ...
  return {
    ...
    ...(land ? { landApproval: { minutes: minutes(now, land.since), approved: Boolean(land.approved), signals: land.signals, evidence: land.evidence } } : {}),
...
function asksOf(ledger: Ledger, now: number): FlowAsk[] {
  return Object.values(ledger.asks)
    ...
    .map((ask) => ({ id: ask.id, kind: ask.kind, fromRole: ask.fromRole, to: ask.to, minutes: minutes(now, ask.openedAt), text: ask.text.split(/\r?\n/).find((line) => line.trim()) ?? "" }));
}
...
function questionsOf(ledger: Ledger, now: number): FlowQuestion[] {
  return Object.values(ledger.questions)
    ...
    .map(({ id, question, why, lane, class: kind, options, recommend, reason, ifSilent, openedAt }) => ({ id, question, why, lane: lane ?? null, class: kind, options, recommend, reason, ifSilent, minutes: minutes(now, openedAt) }));
}

/** Every supervising seat, one per concern; a concern with nobody seated shows its last seat as gone. */
function supervisorsOf(ledger: Ledger, seats: Map<string, SeatView>, now: number, supervises: ReadonlySet<string>, seated: { id: string; role: string }[]): FlowSeat[] {
  ...
  const live = recorded.filter((agent) => seats.has(agent.id)).sort((a, b) => Date.parse(seats.get(b.id)!.updatedAt) - Date.parse(seats.get(a.id)!.updatedAt));
