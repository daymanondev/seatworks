...
| `add_tasks` | Records tasks in the lane in one call, each waiting for what it names, and starts what can start. A task in the lane's copy waits while another holds it, and goes to the Peer kept there unless it is `fresh`. Each brief names the tasks written beside it and what they own, and a kept Peer's says when nobody is. The whole call is refused when two tasks that may run at once own one path, a parallel task owns a one-writer path, a task owns a path outside the lane's write set, or a task takes a path a running task still writes |
