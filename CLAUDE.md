...
- **Never print or cat a file that can hold a key:** `settings.json` under `~/.local/share/seatworks-v2/`
  or `seatworks-v3/`, any project `settings.json`, `~/.paseo/config.json`. Fake keys in tests never
  start with OpenRouter's real key prefix, so a scan for it before a push finds only a real key.
